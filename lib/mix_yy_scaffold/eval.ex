defmodule MixYyScaffold.Eval do
  @moduledoc """
  ...
  defp do_eval_defs(path, config, overrides, args) do
    ...
    user_flags =
      case OptionParser.parse(args, strict: flags) do
        {opts, [], []} ->
          opts
    ...
    user_config =
      ...
      |> MixYyScaffold.Config.apply_overrides(overrides)
  ...
  defp make_env_for(:defs) do
    import MixYyScaffold.Macros, only: [flag: 2, param: 2], warn: false
    __ENV__
  end

  defp make_env_for(:init) do
    import MixYyScaffold.Macros, only: [select: 2], warn: false
    __ENV__
  end
end
